...
public class FakeDataUtils {
  ...
  private static final String[] actions = "登录,搜索,点击,支付,退出".split(",");

  private static final String[] nationalities = "中国,美国,日本,法国,德国,新西兰,澳大利亚".split(",");

  private static int getIndex(int start, int end) {
    return (int) (Math.random() * (end - start + 1) + start);
  }

  public static long buildTelephone() {
    int index = getIndex(0, phonePrefix.length - 1);
    String first= phonePrefix[index];
    String second = valueOf(getIndex(1, 888) + 10000).substring(1);
    String third = valueOf(getIndex(1, 9100) + 10000).substring(1);
    return Long.parseLong(first + second + third);
  }


  public static String buildAddress() {
    int index = getIndex(0, road.length - 1);
    String first = road[index];
    String second = format("%d号", getIndex(11, 150));
    String third = String.format("-%d-%d", getIndex(1, 20), getIndex(1,10));
    return first + second + third;
  }

  public static Pair<String, String> buildNameAndSex() {
    String sex = "男";

    int index = getIndex(0, firstName.length() - 1);
    String first = firstName.substring(index, index + 1);
    int sexIndex = getIndex(0, 1);
    String str = boyName;
    ...
    index = getIndex(0, length - 1);
    String second = str.substring(index, index + 1);
    int hasThird = getIndex(0,1);
    String third = "";
    if(hasThird == 1){
      index = getIndex(0,length - 1);
      third = str.substring(index, index + 1);
    }
    return Pair.of(first + second + third, sex);
  }

  public static String buildName(String firstName, boolean isGirl) {
    String str = isGirl ? girlName : boyName;
    int length = str.length();
    int index = getIndex(0, length - 1);
    String second = str.substring(index, index + 1);
    int hasThird = getIndex(0,1);
    String third = "";
    if(hasThird == 1){
    ...
    return firstName + second + third;
  }

  public static String buildNameName(boolean isGirl) {
    int index = getIndex(0, firstName.length() - 1);
    String first = firstName.substring(index, index + 1);
    String str = isGirl ? girlName : boyName;
    int length = str.length();
  ...
  public static String buildAction() {
    int index = getIndex(0, actions.length - 1);
    return actions[index];
  }

  public static String buildNameNationality() {
    int index = getIndex(0, nationalities.length - 1);
